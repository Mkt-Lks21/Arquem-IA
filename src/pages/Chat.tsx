...
export default function Chat() {
  ...
  const greeting = useMemo(() => {
    ...
    const variants = [
      ...
      "Que insight do seu banco você quer ver?",
    ];
    const index = Math.floor(Math.random() * variants.length);
    return { title: `Olá, ${name}.`, subtitle: variants[index] };
  }, []);

  const suggestions = useMemo(() => {
    const fallback = [
      "Quais tabelas existem no banco?",
      "Mostre os últimos 10 registros de uma tabela.",
      "Quais métricas principais posso acompanhar aqui?",
    ];

    if (!agentTables.length) return fallback;

    const toName = (t: AgentTable) => `${t.schema_name}.${t.table_name}`;
    ...
    return [
      ...
      `Resuma os principais indicadores de ${pick(2)} por mês.`,
    ];
  }, [agentTables]);
